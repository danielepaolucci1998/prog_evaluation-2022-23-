...
# PRESENTAZIONE
# Confronto tra gli algoritmi con le metriche facendo le medie tra soggetti
# Confusion Matrix cumulativa (sommare tutti i valori delle CM relativi ad un algoritmo) 
# Accuratezza per ogni soggetto per ogni algoritmo (figura finale con vaidation e test)
# Algoritmo 1 - subj 11 - controllare il segnale
...
random_state = 42
subj_scores = []
subject_accuracy = [] # Lista con vari parametri riguardo accuratezze
cm_cumulative = np.zeros(5) # Cumulative Confusion Matrix
algoritmo = 5
for subj in range(1, 16):
    ...
    if algoritmo == 1:
        ...
        subject_accuracy.append([subj, np.amax(avg_scores), top_par, accuracy_cv/100, precision, recall, fscore])
        # Soggetto - Max Accuratezza fase Validazione - Grado del modello per Max Accuratezza -
        # - Max Accuratezza fase Test - Precision - Recall - FScore
    ...
    if algoritmo == 5:
        ...
        from sklearn.metrics import precision_recall_fscore_support as score
        [precision, recall, fscore, support] = score(y_test,y_pred,average='macro')
        subject_accuracy.append([subj, np.amax(avg_scores), 0, accuracy_cv/100, precision, recall, fscore])
        # Soggetto - Max Accuratezza fase Validazione - Grado del modello per Max Accuratezza -
        # - Max Accuratezza fase Test - Precision - Recall - FScore
    ...
    cm_cumulative = cm_cumulative + cm
...
subj_test_accuracy = subject_accuracy[:, 3] * 100
plt.figure(figsize = (6.5, 3))
...
average_test_outcome = np.mean(subject_accuracy[:, 1:7], axis=0)
average_test_outcome = np.delete(average_test_outcome, 1)
# Plot Cumulative Confusion Matrix
df_cm_cumulative = pd.DataFrame(cm_cumulative.astype('int'), index = [i for i in list_activities],
                                 columns = [i for i in list_activities])
plt.figure(figsize = (6,4))
sns.set(font_scale=1)
sns.heatmap(df_cm_cumulative, annot=True, annot_kws={"size": 9}, fmt='g', cmap = 'Blues', cbar = False);    
