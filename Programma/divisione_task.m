function tempi_task = divisione_task(dati_imu, FS, subj)
    ...
    if subj==7 | subj==11 % Federica e Giorgia
        [~, tempi_task(3)] = max(svm(tempi_task(2)+115*FS : tempi_task(2)+125*FS)); 
        tempi_task(3) = tempi_task(3) + tempi_task(2)+115*FS - 1;
    else
    ...
    if subj==7 | subj==11
        [~, tempi_task(5)] = max(svm(tempi_task(4)+85*FS : tempi_task(4)+95.5*FS));
        tempi_task(5) = tempi_task(5) + tempi_task(4)+85*FS - 1;
    else
    ...
    if subj==11
        [~, tempi_task(7)] = max(svm(tempi_task(6)+57*FS : tempi_task(6)+63*FS)); 
        tempi_task(7) = tempi_task(7) + tempi_task(6)+57*FS - 1;
    else
