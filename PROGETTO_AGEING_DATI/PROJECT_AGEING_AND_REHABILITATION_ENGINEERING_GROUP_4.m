%% Project of AGEING AND REHABILITATION ENGINEERING
clear
close all
clc
...
doctype = "pdf";
rpt = Report("RESULTS_OF_THE_PATIENT",doctype);
open(rpt);
...
ch5 = Chapter("Frequency Analysis of ECG and PPG signal"); % Capitolo ECG & PPG
...
for i=1:5
    PowBand1_15_ecg(i)=bandpower(VX_01_15s{i,1}, FS, [1,15]);
    PowBand15_30_ecg(i)=bandpower(VX_15_30s{i,1}, FS, [15,30]);
    PowBand30_45_ecg(i)=bandpower(VX_30_45s{i,1},FS,[30 45]);
end

figure()
subplot(3,1,1)
bar(PowBand30_45_ecg)
xlabel('task')
ylabel('Power')
title("POWER BAND ECG - HIGH FREQUENCIES")
subplot(3,1,2)
bar(PowBand15_30_ecg)
xlabel('task')
ylabel('Power')
title("POWER BAND ECG - MEDIUM FREQUENCIES")
subplot(3,1,3)
bar(PowBand1_15_ecg)
xlabel('task')
ylabel('Power')
title("POWER BAND ECG - LOW FREQUENCIES")
...
for i=1:5
    ...
    tabella=table(PowBand1_15_ppg,PowBand15_30_ppg,PowBand30_45_ppg);
end

figure()
subplot(3,1,1)
bar(PowBand30_45_ppg)
xlabel('task')
ylabel('Power')
title("POWER BAND PPG - HIGH FREQUENCIES")
subplot(3,1,2)
bar(PowBand15_30_ppg)
xlabel('task')
ylabel('Power')
title("POWER BAND PPG - MEDIUM FREQUENCIES")
subplot(3,1,3)
bar(PowBand1_15_ppg)
xlabel('task')
ylabel('Power')
title("POWER BAND PPG - LOW FREQUENCIES")
...
%% FREQUENCY ANALYSIS OF ECG AND PPG
...
narx_net.trainParam.min_grad = 0.1
...
vxecg = PPG_SUBsplit{b}(1:LS);

y = con2seq(vxecg');
u = con2seq(M_PSI_st{b}(1:LS));
...
d2 = [1:2];
narx_net = narxnet(d1,d2,10);
narx_net.trainParam.min_grad = 10
[p,Pi,Ai,t,EW,shift] = preparets(narx_net,u,{},y);
narx_net = train(narx_net,p,t,Pi)
yp = sim(narx_net,p,Pi);
t1 = cell2mat(t);
